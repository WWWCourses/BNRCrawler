...
class Scraper:
	...
	def get_pub_date(self, pub_date_div):
	...
	def get_pubs_urls(self):
		pubs_1_1_html = self.soup.select(
			'#module_1_1 [itemtype="http://schema.org/Article"]>div:nth-child(2)')

		pubs_urls = []

		for pub_div in pubs_1_1_html:
			pub_date_div = pub_div.select('[itemprop="datePublished"]')
			pub_date = self.get_pub_date(pub_date_div)
